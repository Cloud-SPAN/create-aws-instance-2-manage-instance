...
On the Billing Dashboard page the region will be set of 'Global' on the top right. You need to change it to 'Ireland'
...
To setup billing alerts select **Billing preferences** on the left navigation pane. Check the following options:
...
To stop (or to perform any other operation on) your instance you need to be in the Instance menu page. Type **ec2** in the AWS search box and press Enter. Check the default region is **Ireland**; set it thus if not.  Then on the left navigation pane, click on **Instances**. 

You should see instance listed with a state of *Pending* or *Running*. Check the box to the left of your instance entry and then click on the drop-down menu **Instance state**. The drop-down menu shows the operations you can perform while your instance is *Running*: Stop instance, Reboot instance and Terminate instance. 
...
Once your instance is running and the its box is ticked, you will see the Public IPv4 DNS address of your instance at the bottom right. Note that, for you to see the Public IPv4 DNS address of your instance your instance must have been checked (blue check box to the left of your instance entry) and the **Details** menu must have been selected.
...
> ## Exercise
> You will need the Public IPv4 DNS address. Make a note of it or copy it to a text file.
{: .challenge}
...
3. **Check that you are in the right folder**
    ...
    You can check if the working directory is set correctly by looking at the file path which is defined to the left of your command prompt (`$`). It should display the second half of the absolute path we wrote down previously, usually starting after your computer's username, and always ending in `/cloud-span-instance`.
...
1. Copy and paste the command in the Code box below to your *terminal*:
    ...
    Now use the back arrow to edit and replace the **NN-NNN-NNN-NN** with the numbers from your instance Public IPv4 DNS.
...
> - The part **-i your-login-key-file.pem** tells `ssh` the identity file it should use to login to your instance. `ssh` will only work if the output of `ls` shows cloud-span-login-key.pem. That is, cloud-span-login-key.pem must be in your working directory.
...
We instructed you to use the t2.micro type of instance so that you don't incur costs under the *Free tier* (for 12 months since you opened your account). This has the following configuration:
